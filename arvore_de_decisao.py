...
class AplicacaoTriagem:
    """
    Classe principal para a aplicação de triagem de sintomas.
    Gerencia a interface gráfica, a lógica da árvore de decisão e os dados do usuário.
    """
    def __init__(self, janela_tk):
        self.janela_principal = janela_tk
        self.janela_principal.title("Triagem de Sintomas")
        self.janela_principal.geometry("700x650") # Aumentei um pouco a altura para mais texto
        ...
        self.dados_usuario = {}
        self.caminho_percorrido = [] # Para logar o caminho de nós
        ...
        self.rotulo_aviso_rodape = tk.Label(self.janela_principal,
                                text="IMPORTANTE: Este sistema NÃO fornece diagnóstico médico.\nProcure um profissional de saúde para diagnósticos e tratamentos.",
                                font=("Arial", 9), fg="red", bg="#f0f0f0", pady=10)
        self.rotulo_aviso_rodape.pack(side=tk.BOTTOM, fill=tk.X)

        # Referências aos widgets dinâmicos
        self.botao_sim = None
        self.botao_nao = None
        ...
        if self.df_resultados is not None and self.arvore_decisao is not None:
            ...
            self.logger.critical("Falha crítica ao carregar dados de configuração. Aplicação será encerrada.")
            messagebox.showerror("Erro Crítico", "Falha ao carregar arquivos de configuração (resultados.json ou arvore_decisao.json).\nVerifique os arquivos e o arquivo 'triagem_log.txt' para detalhes.")
            self.janela_principal.destroy() # Fecha a janela se não puder carregar
    ...
    def _carregar_dados_configuracao(self):
        """Carrega os dados de resultados e a árvore de decisão de arquivos JSON."""
        script_dir = os.path.dirname(os.path.abspath(__file__))
        ...
        try:
            ...
            self.logger.error(f"Arquivo {path_resultados} não encontrado.")
        ...
        try:
            ...
            self.logger.error(f"Arquivo {path_arvore} não encontrado.")
        except json.JSONDecodeError as e:
    ...
    def limpar_frame_conteudo(self):
        """Remove todos os widgets do frame de conteúdo dinâmico."""
    ...
    def processar_no_atual(self, id_no_ou_info_resultado):
        """
        Processa o nó atual da árvore de decisão, exibindo a pergunta/entrada apropriada
        ...
        if isinstance(id_no_ou_info_resultado, dict) and id_no_ou_info_resultado.get('tipo_resultado') == 'direto':
            ...
            self.logger.info(f"Nó de resultado direto: ID {id_res}. Caminho: {' -> '.join(self.caminho_percorrido)}")
        ...
        if not no_info:
            self.logger.error(f"Erro de Navegação: Nó da árvore não encontrado: {id_no}. Caminho: {' -> '.join(self.caminho_percorrido)}")
            messagebox.showerror("Erro de Navegação", f"Nó da árvore não encontrado: {id_no}")
            self.mostrar_tela_inicial()
            return

        self.no_atual_id = id_no
        self.rotulo_pergunta_atual.config(text=no_info.get('texto', "Processando..."))
        self.logger.info(f"Processando nó: {id_no} - '{no_info.get('texto', '')}'")
        ...
        if tipo_no == 'pergunta':
            ...
            if nome_metodo_logica and hasattr(self, nome_metodo_logica):
                ...
                self.logger.error(f"Método de lógica '{nome_metodo_logica}' não encontrado no nó '{id_no}'. Caminho: {' -> '.join(self.caminho_percorrido)}")
                messagebox.showerror("Erro de Configuração da Árvore", f"Método de lógica não definido para o nó {id_no}.")
                self.mostrar_tela_inicial()
        else:
            self.logger.error(f"Tipo de nó desconhecido: {tipo_no} no nó '{id_no}'. Caminho: {' -> '.join(self.caminho_percorrido)}")
            messagebox.showerror("Erro de Nó", f"Tipo de nó desconhecido: {tipo_no}")
            self.mostrar_tela_inicial()

    def _executar_acao_registro(self, acao_registro_info, valor_resposta_booleana):
        """Executa a ação de registrar a resposta do usuário se especificada no nó."""
    ...
    def _montar_pergunta_sim_nao(self, no_info):
        """Monta os botões Sim/Não para um nó do tipo 'pergunta'."""
        ...
        self.botao_nao = tk.Button(self.frame_conteudo, text="Não",
    ...
    def _montar_entrada_numerica(self, no_info):
        """Monta a caixa de entrada para um nó do tipo 'entrada_numerica'."""
        ...
        def acao_enviar(event=None): 
            ...
            try:
                ...
                self.logger.info(f"Entrada numérica registrada: {no_info['chave_dado']} = {valor}")
                if self.janela_principal.winfo_exists():
                    self.janela_principal.unbind('<Return>')
                self.processar_no_atual(no_info['proximo_no'])
            except ValueError:
                self.logger.warning(f"Entrada numérica inválida: '{valor_str}'")
    ...
    def exibir_resultado_final_gui(self, id_resultado_selecionado):
        """Exibe o resultado final da triagem na interface gráfica, incluindo orientações de cuidados."""
        self.limpar_frame_conteudo()
        self.rotulo_pergunta_atual.config(text="--- Triagem Inicial Concluída ---")
        self.logger.info(f"Resultado final exibido: ID {id_resultado_selecionado}. Dados do usuário: {self.dados_usuario}. Caminho final: {' -> '.join(self.caminho_percorrido)}")
        ...
        if resultado_df_filtrado.empty:
            self.logger.error(f"Resultado com ID {id_resultado_selecionado} não encontrado no DataFrame.")
            messagebox.showerror("Erro", f"Resultado com ID {id_resultado_selecionado} não encontrado.")
            self.mostrar_tela_inicial()
            return
        ...
        self.area_texto_resultado_widget.insert(tk.INSERT, f"Com base nas suas respostas, a indicação principal é: {resultado['Resultado_Principal']}\n\n")
        self.area_texto_resultado_widget.insert(tk.INSERT, "Probabilidades Estimadas (hipotéticas):\n")
        ...
        if ("ALERTA" in str(resultado['Resultado_Principal']).upper()) or \
           (pd.notna(resultado['Prob_Aval_Medica']) and resultado['Prob_Aval_Medica'] > 0.20) :
            ...
            self.area_texto_resultado_widget.insert(tk.INSERT, f"ATENÇÃO: Há uma probabilidade considerável ({prob_aval_medica_str}) ou um indicativo de que você deve procurar AVALIAÇÃO MÉDICA PROFISSIONAL.\n\n", "alerta")
        elif pd.notna(resultado['Prob_Outro']) and resultado['Prob_Outro'] > 0.50:
             self.area_texto_resultado_widget.insert(tk.INSERT, f"  - Outra condição / Sintomas inespecíficos: {resultado['Prob_Outro']*100:.0f}%\n")
             self.area_texto_resultado_widget.insert(tk.INSERT, "Seus sintomas são um pouco inespecíficos ou não se encaixam claramente nos padrões mais comuns.\n")
             self.area_texto_resultado_widget.insert(tk.INSERT, "Recomendamos observar a evolução dos sintomas. Se persistirem ou piorarem, procure orientação médica.\n\n")

        self.area_texto_resultado_widget.insert(tk.INSERT, "--- Orientações Gerais de Cuidados ---\n", "subtitulo_cuidados")
        if pd.notna(resultado.get('Orientacoes_Cuidados')) and str(resultado['Orientacoes_Cuidados']).strip():
            ...
            self.area_texto_resultado_widget.insert(tk.INSERT, "Monitore seus sintomas atentamente. Beba líquidos, descanse e alimente-se de forma saudável. Em caso de piora, persistência dos sintomas ou qualquer dúvida, procure orientação médica profissional.\n\n")
        
        self.area_texto_resultado_widget.insert(tk.INSERT, "Lembre-se: Este é um sistema de triagem inicial e NÃO SUBSTITUI UM DIAGNÓSTICO MÉDICO.\n")
        self.area_texto_resultado_widget.insert(tk.INSERT, "Se os sintomas persistirem, piorarem, ou se você tiver qualquer preocupação, procure um profissional de saúde.")
    ...
    # --- Métodos de Lógica da Árvore ---
    def avaliar_com_febre_e_dor_corpo(self):
        """
        Avalia o resultado se o usuário tem febre (sem dificuldade respiratória inicial) E dor no corpo.
        Pode levar ao ID 1 (Gripe Forte) ou a um nó para checar perda de olfato antes do ID 3.
        """
        ...
        if temp >= 38.5:
            ...
            # Considerar se aqui também deveria checar olfato para um ID COVID específico.
            # Por ora, mantém simples, direcionando para Gripe Forte se febre alta + dor corpo.
    ...
    def avaliar_com_febre_sem_dor_corpo_ou_proxima_pergunta(self):
        """
        Avalia se deve ir para próxima pergunta ou resultado, se febre sem dor no corpo (e sem dif. resp. inicial).
        Pode levar a 'no_tosse_produtiva...' (para ID 2) ou a um nó para checar perda de olfato antes de ID 3 ou 8.
        """
        ...
        temp = self.dados_usuario.get('temperatura', 0)
        if temp >= 38.5: 
            # Febre alta sem dor no corpo. Leva à pergunta sobre tosse produtiva (ID 2).
            # Similar ao caso acima, poderia haver uma checagem de olfato antes de direcionar para ID 2,
            # se quiséssemos um ID específico para "Febre Alta + Tosse Produtiva + Perda Olfato".
            return 'no_tosse_produtiva_com_febre_sem_dor'
        else: 
            # Febre baixa/moderada sem dor no corpo.
            # O nó 'no_dor_garganta_febre_baixa_sem_dor_corpo_checar_olfato' já foi inserido
...
if __name__ == "__main__":
    ...
    if app.df_resultados is not None and app.arvore_decisao is not None:
        ...
        print("Encerrando a aplicação devido a erro no carregamento da configuração.")
